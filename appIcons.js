...
export const TaskbarAppIcon = GObject.registerClass(
  ...
  class TaskbarAppIcon extends AppDisplay.AppIcon {
    ...
    _drawRunningIndicator(area, isFocused, targetWidth) {
      ...
      let size = 3.5;
      let startY = areaHeight - size;
      let spacing = Math.ceil(areaWidth / 18);
      let radius = 2.5;
      const dashWidth = 10;
      const metroWidth = (areaWidth / 2.8).toFixed(0);
      if (!area._currentWidths || area._currentWidths.length !== n) {
        area._currentWidths = Array(n).fill(dashWidth);
      }
      let drawIndicator = (x, w) => {
        cr.newPath();
        cr.moveTo(x + radius, 0);
        cr.lineTo(x + w - radius, 0);
        cr.arc(x + w - radius, radius, radius, -Math.PI / 2, 0);
        cr.lineTo(x + w, size - radius);
        cr.arc(x + w - radius, size - radius, radius, 0, Math.PI / 2);
        cr.lineTo(x + radius, size);
        cr.arc(x + radius, size - radius, radius, Math.PI / 2, Math.PI);
        cr.lineTo(x, radius);
      ...
      cr.save();
      cr.translate(0, startY);
      cr.setSourceColor(bodyColor);

      if (!isFocused) {
        let totalWidth = n * dashWidth + (n - 1) * spacing;
        let offsetX = (areaWidth - totalWidth) / 2;
        cr.translate(offsetX, 0);

        for (let i = 0; i < n; i++) {
          cr.newSubPath();
          // Mevcut width'i sıfırlamadan kullan, küçülme sırasında flicker olmaz
          if (targetWidth !== undefined) {
            // Hedef width ile mevcut width arasında karşılaştır, küçülürken sıfırlama yok
            area._currentWidths[i] =
              targetWidth < area._currentWidths[i]
                ? area._currentWidths[i]
                : targetWidth;
          }
          drawIndicator(i * (dashWidth + spacing), area._currentWidths[i]);
        }
      } else {
        let offsetX = (areaWidth - metroWidth) / 2;
        cr.translate(offsetX, 0);
        cr.newSubPath();

        if (targetWidth !== undefined) {
          area._currentWidths[0] =
            targetWidth < area._currentWidths[0]
              ? area._currentWidths[0]
              : targetWidth;
        } else {
          area._currentWidths[0] = metroWidth;
        }

        drawIndicator(0, area._currentWidths[0]);
      }
    ...
    _resetDots(ignoreSizeReset) {
      let sizeProp = true ? "width" : "height";
    ...
    _displayProperIndicator() {
      ...
      if (!this._isGroupApps) {
        ...
        // only animate if going from a wide style to a narrow style indicator or vice-versa
    ...
    _isFocusedWindow() {
      ...
      while (focusedWindow) {
        if (focusedWindow == this.window) return true;
