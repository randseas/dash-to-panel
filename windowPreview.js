...
export const PreviewMenu = GObject.registerClass(
  ...
  class PreviewMenu extends St.Widget {
    ...
    _updatePosition() {
      ...
      let sourceAllocation = Utils.getTransformedAllocation(
        this.currentAppIcon
      );
...
export const Preview = GObject.registerClass(
  ...
  class Preview extends St.Widget {
    _init(previewMenu) {
      ...
      closeButton.add_child(
        new St.Icon({ icon_name: "window-close-symbolic" })
      );
    ...
    adjustOnStage() {
      ...
      if (!headerHeight) {
        ...
        if (isTopHeader) {
          closeButtonBorderRadius += isLeftButtons
            ? "0 0 4px 0;"
            : "0 0 0 4px;";
        } else {
          closeButtonBorderRadius += isLeftButtons
    ...
    getSize() {
      ...
      binHeight =
        Math.max(binHeight, this.cloneHeight + this._padding * 2) +
        headerHeight;
